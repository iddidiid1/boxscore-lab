...
- [x] Page CSS mainly controls layout, spacing, responsive behavior, and page-specific exceptions.
- [x] A future visual redesign can be completed primarily by changing tokens and shared component styles.
- [x] No migration changes page behavior or business logic.
...
- [x] Document any intentional one-off literal value next to its usage.
...
- [x] Verify that Mantine props such as `color`, `radius`, `c`, `bg`, and inline `styles` do not bypass semantic styling without a page-specific reason.
...
- [x] Consolidate status badge/chip styling around semantic states rather than page-specific colors.
- [x] Keep shared primitives small; do not introduce a component abstraction for a pattern used only once.
...
- [x] Batch 1 - Team Editor (`/teams/new` and `/teams/:slug/manage`).
...
- [x] Batch 2 - Player Statistics (`PlayersPage` and `PlayerDetailPage`).
...
- [x] Batch 3 - Match History and Entry (`MatchesPage`, `CreateMatchPage`, `EditMatchPage`, and `MatchDetailPage`).
  ...
  - [x] Complete desktop/mobile and interactive-state browser verification when browser tooling is available.
- [x] Batch 4 - Team Browsing (`TeamsPage` and `TeamDetailPage`).
  ...
  - [x] Confirm the routes' default visual state through user review.
  - [x] Complete desktop/mobile and interactive-state browser verification when browser tooling is available.
- [x] Batch 5 - Event Workflow (event list, form, detail, outcomes, and direct components).
...
- [x] Remove obsolete selectors after each page is migrated.
...
### Editorial Scoreboard M6 - Scenario Sweep and Cleanup

- [x] Sweep representative Team, Player, Event, and Match Overview, Detail,
  Create/Edit, and Results/Awards routes at wide, 760px, and 560px viewports.
- [x] Confirm no whole-page horizontal overflow and retain local table
  overflow for dense data at narrow widths.
- [x] Remove the unused placeholder component, obsolete universal-card and
  telemetry selectors, hidden Eyebrow markup, and redundant `app-panel`
  surface aliases.
- [x] Confirm no proposal-only token names or obsolete universal-card rules
  remain in application source.
- [x] Audit remaining Mantine visual props and inline styles as semantic
  Alert/Skeleton roles, dynamic Team data, or proportional/structural values.
- [x] Inspect the running application console across the scenario sweep; no
  application errors or warnings were reported.
...
- [x] Require new shared visual patterns to reuse an existing primitive or explain why a new variant is needed.
...
- [x] Run frontend tests when present; the MVP workflow currently specifies manual smoke testing only.
- [x] Build the frontend.
- [x] Visually compare affected pages at desktop and mobile widths.
- [x] Check default, hover, focus-visible, active, disabled, loading, validation, and destructive states where applicable.
- [x] Check table overflow and dense statistical displays.
- [x] Check text contrast and visible keyboard focus.
- [x] Confirm no unrelated files or behaviors changed.
