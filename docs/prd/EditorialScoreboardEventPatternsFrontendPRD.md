...
- `apps/frontend/src/styles/variables.css`
  ...
  - 新增 Black Metal Plaque 专属的 solid near-black material、
...
- 一个 `EventPlayerAwardsPanel` root 构成完整 Black Metal Plaque，是该 section
  ...
  card 外观。
- plaque header 内显示 `Player Awards`；cut corners、engraved rules 和 corner
  marks 使用 CSS/pseudo-elements，不能成为额外可读内容；不使用 repeating
  stripes 或 visible brushed grain。
- MVP：
...
- Black Metal Plaque 不排序、不去重、不补齐 awards；只按 type 分组并保留每组
  API 相对顺序。
- 不新增 image/font/material fetch；insignia facets 由 CSS 完成。
...
- 装饰 rails/facets/corner marks 使用 `aria-hidden` 或 CSS
