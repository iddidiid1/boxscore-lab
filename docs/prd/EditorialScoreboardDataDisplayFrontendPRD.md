...
共享视觉与小型可复用组件：表格使用 38px Compact Dark Glass Data Bay，Filter Bar
...
| Data table | quiet header、38px body baseline、水平 divider、numeric typography |
...
- `MatchStatsInputTable` 保持 Editor surface 和表单语法；它不变成完整的 read-only
  Data Bay，但 table geometry、header、divider、38px baseline 与 local overflow
...
- body row baseline 为 38px；内容换行或可访问文本需要更高时允许自然增长，
  不裁切文字来强制高度。
...
- Championship Gold Winner row 使用 restrained directional tint/trace；
