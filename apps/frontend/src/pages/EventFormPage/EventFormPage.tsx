...
export function EventFormPage({ eventId, mode }: { eventId?: string; mode: "create" | "edit" }) {
  ...
  async function archive() { if (!event || !window.confirm("Archive this event? Historical data will remain available.")) return; setSubmitting(true); try { await archiveEvent(event.slug); navigate("/events"); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to archive event."); setSubmitting(false); } }
  if (loading) return <Loader size="sm"/>; if (mode === "edit" && !event && error) return <Alert color="red">{error}</Alert>;
  return <EventForm event={event} mode={mode} teams={teams} readOnly={Boolean(event?.archivedAt) || event?.status === "COMPLETED"} submitting={submitting} error={error} onCancel={() => navigate(event ? `/events/${event.slug}` : "/events")} onSave={save} onStatusChange={changeStatus} onArchive={event?.archivedAt ? undefined : archive}/>;
}
