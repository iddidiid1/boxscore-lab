...
export function MatchBoxScoreTable({ otherStats, players, score, teamColor, title }: Props) {
  return (
    <Box className="match-box-score-card app-data-bay">
      ...
      <Box className="match-box-score-scroll app-table-wrap">
        <Table className="match-box-score-table app-data-table" highlightOnHover>
          ...
          <Table.Tbody>
            ...
            <Table.Tr className="match-box-score-other-row app-data-row--operational"><Table.Td>—</Table.Td><Table.Td>Other</Table.Td><Table.Td>{otherStats.points}</Table.Td><Table.Td>{otherStats.rebounds}</Table.Td><Table.Td>{otherStats.assists}</Table.Td><Table.Td>{otherStats.fieldGoalsMade}</Table.Td><Table.Td>{otherStats.fieldGoalsAttempted}</Table.Td><Table.Td>{percent(otherStats.fieldGoalPercentage)}</Table.Td><Table.Td>{otherStats.threePointersMade}</Table.Td><Table.Td>{otherStats.threePointersAttempted}</Table.Td><Table.Td>{percent(otherStats.threePointPercentage)}</Table.Td><Table.Td>—</Table.Td><Table.Td>—</Table.Td></Table.Tr>
            <Table.Tr className="match-box-score-total-row">
              <Table.Td>—</Table.Td>
              <Table.Td>Team Total</Table.Td>
              <Table.Td>{score}</Table.Td>
              {Array.from({ length: 10 }, (_, index) => <Table.Td key={index}>—</Table.Td>)}
            </Table.Tr>
          </Table.Tbody>
        </Table>
      </Box>
